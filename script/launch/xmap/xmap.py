...
def on_socketio(*args):
    if args[0]["topic"] == "goal_info":
        ...
        Global_Variable['goal']['head'] = float(args[0]['pose']['head']) #rad
        ...
        q = tf.transformations.quaternion_from_euler(0, 0, Global_Variable['goal']['head'])
	data.goal.target_pose.pose.orientation.x = q[0]
	data.goal.target_pose.pose.orientation.y = q[1]
	data.goal.target_pose.pose.orientation.z = q[2]
	data.goal.target_pose.pose.orientation.w = q[3]
    ...
    if args[0]["topic"] == "command":
        if args[0]["key"] == "cancel":
            data = GoalID()
            Global_Variable['ros']['cancel_publisher'].publish(data)
...
def ros_move_base_status_callback(data):
    ...
    state_tabl = ['PENDING','RUNNING','PREEMPTED','SUCCEEDED','ABORTED','REJECTED','PREEMPTING','RECALLING','RECALLED','LOST']
...
if __name__ == "__main__":
    ...
    io_helper = IOHelper('localhost',80)
    ...
    ros_cancel_publisher = rospy.Publisher('/move_base/cancel', GoalID, queue_size=1)
    Global_Variable['ros']['goal_publisher'] = ros_goal_publisher
    Global_Variable['ros']['cancel_publisher'] = ros_cancel_publisher

    rospy.Subscriber("/map", OccupancyGrid, ros_map_callback)
    rospy.Subscriber("/cmd_vel", Twist, ros_vel_callback)
